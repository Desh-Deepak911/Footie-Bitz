      ...
      <div className={`flex flex-wrap ${compact ? "gap-1" : "gap-2"}`}>
        {ordered.map((provider) => (
          <div
            ...
            className={
              compact
                ? `inline-flex max-w-full items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-medium ring-1 ${providerChipClass(provider.priority)}`
                : `inline-flex max-w-full items-center gap-2 rounded-full px-3 py-1.5 text-xs font-medium ring-1 transition hover:ring-border/35 focus-within:ring-accent/30 ${providerChipClass(provider.priority)}`
            }
          >
            ...
            {provider.priority === "primary" ? (
              <span
                className={
                  compact
                    ? `${studioCardTag} bg-accent/20 text-accent`
                    : "shrink-0 rounded-full bg-accent/20 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-accent"
                }
              >
                Primary
              </span>
            ...
            <span
              className={
                compact
                  ? `${studioCardTag} bg-black/10 text-muted`
                  : "shrink-0 rounded-full bg-black/10 px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wide text-muted"
              }
            >
              Planning
            </span>
