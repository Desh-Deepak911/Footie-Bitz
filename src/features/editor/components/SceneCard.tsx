...
import type { CaptionMode, FootieScene, SceneImage, SceneImageFitMode, SceneType } from "@/features/story/types";
...
export default function SceneCard({
  ...
  const handleFitModeChange = (fitMode: SceneImageFitMode) => {
    activateScene();
    onImageSettingsChange(scene.id, { fitMode });
  };

  const handleImageMotionChange = (patch: Partial<NonNullable<SceneImage["imageMotion"]>>) => {
    activateScene();
    onImageSettingsChange(scene.id, { imageMotion: patch });
  };
  ...
  return (
    <article
      className={studioStoryboardCard}
            ...
            <div className="relative mx-auto aspect-[9/16] max-h-56 w-full max-w-[11rem] overflow-hidden sm:max-h-64 sm:max-w-[12rem]">
              {sceneHasImage(scene) ? (
                <MediaPicker
                  scene={scene}
                  alt={`${title} visual`}
                  onInteractionStart={activateScene}
        ...
        <section aria-label="Scene caption">
          ...
          {isSubtitlesMode ? (
            <>
              ...
              <div className="mt-3">
                <label htmlFor={`subtitle-text-${scene.id}`} className={studioFieldLabel}>
                  Subtitle Text
                </label>
                <textarea
                  id={`subtitle-text-${scene.id}`}
