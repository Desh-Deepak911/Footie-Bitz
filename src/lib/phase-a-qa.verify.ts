...
const studioUi = readFileSync(join(root, "src/lib/studioUi.ts"), "utf8");
...
const sceneCaptionOverlay = readFileSync(
  join(root, "src/features/editor/components/SceneCaptionOverlay.tsx"),
  "utf8",
);
...
const sceneStoryboardCard = readFileSync(
  join(root, "src/features/editor/components/SceneCard.tsx"),
  "utf8",
);
const storyReview = readFileSync(join(root, "src/components/StoryReview.tsx"), "utf8");
const voiceover = readFileSync(join(root, "src/lib/voiceover.ts"), "utf8");
...
test("generated caption mode uses inline preview path unchanged", () => {
  ...
  assert.match(videoPreview, /showSceneCaption && !isNarrationSubtitles/);
...
test("only the first short subtitle chunk is shown when timing is omitted", () => {
  assert.match(sceneCaptionOverlay, /getDisplayCaption\(scene\)/);
...
test("subtitle mode edits subtitleText in storyboard card", () => {
  assert.match(sceneStoryboardCard, /Subtitle Text/);
  assert.match(sceneStoryboardCard, /subtitleEditorValue/);
  assert.match(sceneStoryboardCard, /scene\.subtitleText \|\| scene\.narration/);
  assert.match(sceneStoryboardCard, /onUpdate\(\{ subtitleText: e\.target\.value \}\)/);
  assert.match(
    sceneStoryboardCard,
    /Subtitles are based on narration, but you can edit them for better on-screen readability/,
  );
});

test("subtitle effect selector appears only in subtitles mode", () => {
  assert.match(sceneStoryboardCard, /isSubtitlesMode \?/);
  assert.match(sceneStoryboardCard, /SubtitleEffectControl/);
  assert.match(sceneStoryboardCard, /onUpdate\(\{ subtitleEffect: effect \}\)/);
  const controlIndex = sceneStoryboardCard.indexOf("<SubtitleEffectControl");
  assert.ok(controlIndex >= 0);
  const subtitlesBranch = sceneStoryboardCard.lastIndexOf("isSubtitlesMode ? (", controlIndex);
  assert.ok(subtitlesBranch >= 0);
});
...
test("switching back to generated preserves subtitleText and generated caption", () => {
  assert.match(sceneStoryboardCard, /onUpdate\(\{ captionMode: mode \}\)/);
  assert.doesNotMatch(
    sceneStoryboardCard,
...
test("narration remains separate from subtitle editing", () => {
  assert.doesNotMatch(sceneStoryboardCard, /onUpdate\(\{ narration:/);
  assert.match(storyReview, /story\.narration/);
  assert.match(storyReview, /onStoryChange\(\{ \.\.\.story, narration: e\.target\.value \}\)/);
  assert.match(sceneStoryboardCard, /Narration timing/);
  assert.doesNotMatch(sceneStoryboardCard, /id=\{`story-narration/);
});
...
test("export and preview skip transition items as video segments", () => {
  ...
  const timelineEditor = readFileSync(
    join(root, "src/features/editor/components/TimelineEditor.tsx"),
    "utf8",
  );
  assert.match(timelineEditor, /isTransitionTimelineItem/);
