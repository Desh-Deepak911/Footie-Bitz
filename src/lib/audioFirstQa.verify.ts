...
test("StoryComposer collects prompt and scene count", () => {
  const composer = readSrc("src/components/StoryComposer.tsx");
  const page = readSrc("src/app/page.tsx");

  assert.match(composer, /sceneCount/);
  assert.match(composer, /onSceneCountChange/);
  assert.match(composer, /topic/);
  assert.match(page, /sceneCount/);
...
test("captions and subtitles paths remain intact", () => {
  ...
  assert.match(sceneCard, /captionMode/);
  assert.match(sceneCard, /subtitleText/);
